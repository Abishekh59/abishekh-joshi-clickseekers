...
const toAbsoluteImageUrl = (
  urlOrObj:
    | string
    | { image_url?: string | null; image_id: number }
    | null
    | undefined,
) => {
...
export default function PhotographerProfile() {
    ...
    const fetchAllData = async () => {
      try {
        ...
        const profileRes = await fetch(
          `${API_HOST}/api/photographer/${photographerId}`,
        );
        ...
        const portfolioRes = await fetch(
          `${API_HOST}/api/photographer/${photographerId}/portfolio/images`,
        );
        ...
        if (token) {
          const [favIdsRes, likesRes, savesRes] = await Promise.all([
            apiService.getFavoritePhotographerIds(token),
            apiService.getUserLikes(token),
            apiService.getUserSaves(token),
          ]);

          if (favIdsRes.success && favIdsRes.data) {
            setIsFavorite(favIdsRes.data.favoriteIds.includes(photographerId));
          }

          // Map liked and saved image IDs to record for quick lookup
          const likedMap: Record<number, boolean> = {};
          (likesRes.data?.likedImageIds || []).forEach((id: number) => {
            likedMap[id] = true;
          });
          setImageLikes(likedMap);

          const savedMap: Record<number, boolean> = {};
          (savesRes.data?.savedImageIds || []).forEach((id: number) => {
            savedMap[id] = true;
          });
          setImageBookmarks(savedMap);
        }
      } catch (e: any) {
  ...
  const toggleImageLike = async (index: number) => {
    ...
    try {
      const token = await storage.getToken();
      if (!token) {
        Alert.alert("Login Required", "Please login to like images");
        return;
      }

      // Optimistic update
      const newLikedState = !imageLikes[image.image_id];
      setImageLikes((prev) => ({ ...prev, [image.image_id]: newLikedState }));

      const result = await apiService.likePortfolioImage(image.image_id, token);
      if (result.data) {
        setImageLikes((prev) => ({
          ...prev,
          [image.image_id]: !!result.data.isLiked,
        }));
      }
    } catch (e) {
  ...
  const toggleImageBookmark = async (index: number) => {
    const image = portfolioImages[index];
    if (!image) return;
    ...
    try {
      const token = await storage.getToken();
      if (!token) {
        Alert.alert("Login Required", "Please login to save images");
        return;
      }

      // Optimistic update
      const newSavedState = !imageBookmarks[image.image_id];
      setImageBookmarks((prev) => ({
        ...prev,
        [image.image_id]: newSavedState,
      }));

      const result = await apiService.toggleImageSave(image.image_id, token);
      if (result.success) {
        setImageBookmarks((prev) => ({
          ...prev,
          [image.image_id]: !!result.data?.isSaved,
        }));
        Alert.alert(
          "Success",
          result.data?.isSaved ? "Saved to portfolio" : "Removed successfully",
        );
      }
    } catch (e) {
      ...
      setImageBookmarks((prev) => ({
        ...prev,
        [image.image_id]: !prev[image.image_id],
      }));
    }
  };
  ...
  const TabButton = ({
    ...
    tab: "Portfolio" | "Packages" | "Reviews";
  }) => {
    ...
    return (
      <TouchableOpacity
        ...
        style={[
          styles.tabBtn,
          active ? { borderBottomColor: primary, borderBottomWidth: 2 } : null,
        ]}
      >
        <ThemedText
          type="sm"
          weight="extrabold"
          style={{ color: active ? primary : gray400 }}
        >
          {tab}
        </ThemedText>
      </TouchableOpacity>
    ...
    return (
      ...
      >
        ...
        <TouchableOpacity
          ...
          onPress={() => router.back()}
        >
          <ThemedText type="sm" weight="extrabold" style={{ color: "#fff" }}>
            Go Back
          </ThemedText>
        </TouchableOpacity>
      </View>
  ...
  return (
    <View style={[styles.container, { backgroundColor: background }]}>
      ...
      >
        ...
        <ThemedText type="base" weight="extrabold" style={{ color: gray900 }}>
        ...
        >
          <Ionicons name="flag-outline" size={20} color={gray900} />
        </TouchableOpacity>
      </View>
      ...
      >
        ...
        <View style={styles.profileRow}>
          <View style={styles.avatarWrap}>
            ...
            toAbsoluteImageUrl(photographer.profile_image) ? (
              <Image
                source={{
                  uri: toAbsoluteImageUrl(photographer.profile_image)!,
                }}
                style={styles.avatar}
              />
            ) : (
              ...
              >
                <ThemedText type="3xl" weight="bold" style={{ color: gray600 }}>
          ...
          <View style={{ flex: 1, marginLeft: 14 }}>
            ...
            <ThemedText
              type="xs"
              weight="bold"
              style={{ color: primary, marginTop: 1, marginBottom: 4 }}
            >
            ...
            <View style={styles.infoRow}>
              ...
              <ThemedText
                type="xs"
                weight="bold"
                style={{ color: gray600, marginLeft: 6 }}
                numberOfLines={1}
              >
                {photographer.email}
              </ThemedText>
        ...
        <View style={styles.statsRow}>
          ...
          <View style={styles.statCard}>
            ...
            >
              ...
              <ThemedText type="xs" weight="bold" style={{ color: gray500 }}>
                {photographer.points?.toLocaleString() || "0"} pts
              </ThemedText>
            </View>
          </View>
          ...
          <View style={styles.statCard}>
            ...
            >
              {photographer.total_reviews !== undefined
            ...
            renderItem={({ item, index }) => (
              ...
              >
                {toAbsoluteImageUrl(item) && (
                  <Image
                    source={{ uri: toAbsoluteImageUrl(item)! }}
            ...
            ListEmptyComponent={
              <View style={styles.emptyContainer}>
                ...
                <ThemedText
                  type="sm"
                  weight="bold"
                  style={{ color: gray400, marginTop: 12 }}
                >
                  ...
                  <View style={styles.packageTop}>
                    <View style={{ flex: 1 }}>
                      ...
                      <ThemedText
                        type="xs"
                        weight="extrabold"
                        style={{ color: gray500, marginTop: 2 }}
                      >
                  ...
                  <View style={{ marginTop: 10, rowGap: 8 }}>
                    {pkg.description ? (
                      <ThemedText
                        type="sm"
                        weight="bold"
                        style={{ color: gray600, lineHeight: 20 }}
                      >
                        {pkg.description}
                      </ThemedText>
                    ) : null}
              ...
              photographer.reviews.map((r: any) => (
                ...
                >
                  <View style={styles.reviewHeader}>
                    {r.reviewer?.profile_image &&
                    toAbsoluteImageUrl(r.reviewer.profile_image) ? (
                      <Image
                        source={{
                          uri: toAbsoluteImageUrl(r.reviewer.profile_image)!,
                        }}
                  ...
                  <ThemedText
                    type="sm"
                    weight="bold"
                    style={{ color: gray600, lineHeight: 18 }}
                  >
                    {r.comment}
                  </ThemedText>
                </View>
      ...
      <View
        style={[
          styles.bottomBar,
          { backgroundColor: background, borderTopColor: "#f3f4f6" },
        ]}
      >
        <TouchableOpacity
          onPress={async () => {
            let wasFavorite = isFavorite; // Declare outside try-catch for error handling
            try {
              const token = await storage.getToken();
              if (!token) {
                Alert.alert(
                  "Error",
                  "You must be logged in to favorite a photographer.",
                );
                return;
              }
              ...
              const res = await apiService.toggleFavoritePhotographer(
                photographerId,
                token,
              );
              if (res.success) {
                ...
                if (res.data?.isFavorited) {
                  ...
                  Alert.alert(
                    "🗑️ Removed",
                    "Photographer removed from your favorites",
                  );
                }
              }
            } catch (err: any) {
        ...
        >
          <Ionicons
            ...
            size={20}
            color={isFavorite ? "#ef4444" : gray500}
          />
        </TouchableOpacity>
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            const pkg =
              selectedPackage ||
              (photographer.packages && photographer.packages[0]);
            if (!pkg) {
              Alert.alert(
                "Notice",
            ...
            const packagesJson = photographer.packages
              ? JSON.stringify(photographer.packages)
              : "";
            router.push({
        ...
        >
          <ThemedText type="base" weight="extrabold" style={{ color: "#fff" }}>
            Book Now
          </ThemedText>
        </TouchableOpacity>
      </View>
      ...
      >
        <View style={styles.modalOverlay}>
          ...
          />
          <View
            style={[styles.imageModalCard, { backgroundColor: background }]}
          >
            <View style={styles.imageModalHeader}>
              ...
              <ThemedText
                type="sm"
                weight="extrabold"
                style={{ flex: 1, textAlign: "center", color: gray900 }}
                numberOfLines={1}
              >
                {photographer.full_name}
              </ThemedText>
            ...
            {selectedImageIndex !== null &&
              toAbsoluteImageUrl(portfolioImages[selectedImageIndex]) && (
                <Image
                  source={{
                    uri: toAbsoluteImageUrl(
            ...
            <View style={styles.postActions}>
              <TouchableOpacity
                onPress={() =>
                  selectedImageIndex != null &&
                  toggleImageLike(selectedImageIndex)
                }
              >
                <Ionicons
                  name={
                    selectedImageIndex != null &&
                    portfolioImages[selectedImageIndex] &&
                    imageLikes[portfolioImages[selectedImageIndex].image_id]
                      ? "heart"
                      : "heart-outline"
                  }
                  size={24}
              ...
              <TouchableOpacity
                onPress={() =>
                  selectedImageIndex != null &&
                  toggleImageBookmark(selectedImageIndex)
                }
              >
                <Ionicons
                  name={
                    selectedImageIndex != null &&
                    portfolioImages[selectedImageIndex] &&
                    imageBookmarks[portfolioImages[selectedImageIndex].image_id]
                      ? "bookmark"
                      : "bookmark-outline"
                  }
                  size={24}
            ...
            {selectedImageIndex !== null && (
              <View style={styles.captionBox}>
                <ThemedText type="sm" weight="bold" style={{ color: gray700 }}>
                  {portfolioImages[selectedImageIndex].title ||
                    "Untitled Portfolio Piece"}
                </ThemedText>
                {portfolioImages[selectedImageIndex].description ? (
                  <ThemedText
                    type="sm"
                    weight="bold"
                    style={{ color: gray600, lineHeight: 20, marginTop: 4 }}
                  >
                    {portfolioImages[selectedImageIndex].description}
                  </ThemedText>
                ) : null}
      ...
      >
        ...
        >
          <View
            style={[styles.reportModalContent, { backgroundColor: background }]}
          >
            <View style={styles.reportModalHeader}>
              ...
              >
                <Ionicons
                  name="close"
                  size={24}
                  color={submittingReport ? "#d1d5db" : gray900}
                />
              </TouchableOpacity>
            </View>
            ...
            <ThemedText
              type="sm"
              weight="bold"
              style={{ color: gray700, marginBottom: 12 }}
            >
            ...
            <TextInput
              style={[
                styles.reportInput,
                { borderColor: "#e5e7eb", color: gray900 },
              ]}
...
const styles = StyleSheet.create({
  ...
  reportModalBtnCancel: {
    backgroundColor: "#f3f4f6",
    borderWidth: 1,
    borderColor: "#e5e7eb",
  },
  reportModalBtnSubmit: {
