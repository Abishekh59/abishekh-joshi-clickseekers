...
export const awardPoints = async ({
  ...
  try {
    ...
    if (!reward) {
      ...
      if (!rookieBadge) {
        console.error(
          "[PointsService] No badges found in database. Please seed badges.",
        );
        return;
      }
    ...
    if (!newBadge) {
      console.error(
        "[PointsService] Could not determine badge for points:",
        newTotalPoints,
      );
      return;
    }
    ...
    try {
      ...
      console.error(
        "[PointsService] Error sending reward notification:",
        notifError,
      );
    ...
    console.log(
      `[PointsService] Awarded ${points} points to user ${userId} for: ${reason}. New total: ${newTotalPoints}, Badge: ${newBadge.badge_name}`,
    );
  } catch (error) {
...
export const POINT_CONFIG = {
  ...
  SAVE_POST: 2,
  UNSAVE_POST: -2,
  SAVE_PROFILE_FIRST: 5,
  SAVE_PROFILE_REPEATED: 5,
 ...
 */
export const awardPhotoUploadPoints = async (
  userId: string,
): Promise<boolean> => {
  try {
    const now = new Date();
    ...
    if (tracking.upload_count >= POINT_CONFIG.PHOTO_UPLOAD_MAX_WEEKLY) {
      console.log(
        `[PointsService] Weekly upload limit reached for user ${userId}`,
      );
      return false;
    }
 ...
 */
export const handlePhotoLike = async (
  imageId: number,
  userId: string,
 ...
 */
export const handlePhotoSave = async (
  imageId: number,
  userId: string,
  photographerId: string,
  isSave: boolean,
  ...
  try {
    ...
    // Get the user's name who is saving/unsaving
    const user = await prisma.user.findUnique({
      where: { user_id: userId },
    ...
    if (isSave) {
      await awardPoints({
        ...
        reason: `Your post was saved by ${userName}`,
      });
    } else {
      await awardPoints({
        userId: photographerId,
        points: POINT_CONFIG.UNSAVE_POST,
        reason: `Your post was unsaved by ${userName}`,
      });
    }
  } catch (error) {
 ...
 */
export const deductCommentPoints = async (
  photographerId: string,
  commentId: number,
): Promise<void> => {
  await awardPoints({
 ...
 */
export const handlePhotographerSave = async (
  photographerId: string,
  clientId: string,
  isSave: boolean,
): Promise<void> => {
    ...
    if (isSave) {
      ...
      if (!existing) {
        ...
        await awardPoints({
          ...
          reason: `Your profile was saved by ${clientName}`,
        });
      } else {
        // Repeated save
        // Get the client's name
        ...
        await awardPoints({
          ...
          reason: `Your profile was saved again by ${clientName}`,
        });
      }
    }
  } catch (error) {
...
export const trackLogin = async (userId: string): Promise<void> => {
  try {
    ...
    const lastLoginDate = new Date(
      tracking.last_login.getFullYear(),
      tracking.last_login.getMonth(),
      tracking.last_login.getDate(),
    );
...
export const getLeaderboard = async (
  ...
  try {
    ...
    return await Promise.all(
      historyAgg.map(async (item: any, index: number) => {
