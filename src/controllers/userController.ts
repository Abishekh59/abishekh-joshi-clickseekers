...
export const login = catchAsync(async (req: Request, res: Response) => {
  ...
  if (user.kyc_verification) {
    ...
    if (user.kyc_verification.status === 'APPROVED' && user.kyc_verified === true) {
      kycStatus = 'verified';
  ...
  // Use specialization from user field, fallback to derived or default
  let specialization = user.specialization || 'Professional Photographer';
  ...
  // Fetch badge from Reward/Badge
  let badge = 'Elite Photographer';
  const reward = await prisma.reward.findUnique({
    where: { user_id: user.user_id },
    include: { badge: true },
  });
  if (reward && reward.badge && reward.badge.badge_name) {
  ...
  const total_bookings = bookingsAgg._count.booking_id || 0;
  const earnings = bookingsAgg._sum.amount ? Number(bookingsAgg._sum.amount) : 0;

  // Fetch location from latest KYC
  ...
  if (!user.location) {
    ...
    if (latestKyc) {
      ...
      location = [address_city, address_district, address_province].filter(Boolean).join(', ') || location;
    }
  }
...
export const verifyPasswordResetOTP = catchAsync(async (req: Request, res: Response) => {
  if (withCors(req, res)) return;
...
export const updatePhotographerProfile = catchAsync(async (req: Request, res: Response) => {
  const authUser = req.user;
  ...
  if (authUser.role !== 'PHOTOGRAPHER') {
    return res.status(403).json({ success: false, message: 'Only photographers can update this profile section' });
  }

  const { full_name, phone, profile_image, bio, location, specialization } = req.body;
...
export const submitKycForm = catchAsync(async (req: Request, res: Response) => {
  ...
  if (!dbUser.email_verified) {
    return res.status(403).json({ success: false, message: 'Please verify your email before submitting KYC' });
  }
  ...
  if (!full_name || !contact_number || !email || !address_city || !address_district || !address_province) {
  ...
  if (!document_type || !allowedDocumentTypes.includes(document_type)) {
    return res.status(400).json({ success: false, message: 'document_type must be one of CITIZENSHIP, PASSPORT, DRIVING_LICENSE' });
  }

  if (!document_number || !document_front_url) {
    return res.status(400).json({ success: false, message: 'Document number and front image are required' });
  }

  if (!(consent_confirmed && consent_verify && consent_false_info)) {
    return res.status(400).json({ success: false, message: 'All declarations and consents must be accepted' });
  }
  ...
  if (resolvedRole !== dbUser.role) {
    return res.status(400).json({ success: false, message: 'Provided user_role does not match account role' });
  }
...
export const reviewKyc = catchAsync(async (req: Request, res: Response) => {
  ...
  if (authUser.role !== 'ADMIN') {
    return res.status(403).json({ success: false, message: 'Only admins can review KYC submissions' });
  }

  const { kyc_id, status, remarks } = req.body;

  if (!kyc_id || !['APPROVED', 'REJECTED'].includes(status)) {
    return res.status(400).json({ success: false, message: 'kyc_id and status (APPROVED|REJECTED) are required' });
  }
  ...
  // Update user flag based on decision
  await prisma.user.update({
    where: { user_id: updated.user_id },
    data: {
      kyc_verified: status === 'APPROVED',
      kyc_document_url: status === 'APPROVED' ? updated.document_front_url : null,
    },
  });

  res.json({ success: true, message: `KYC ${status.toLowerCase()}`, data: updated });
});
...
export const uploadAvatar = catchAsync(async (req: Request, res: Response) => {
  ...
  if ((req as any).io) {
    ...
    // Also emit photographer_updated if user is a photographer
...
export const uploadKycDocuments = catchAsync(async (req: Request, res: Response) => {
  const authUser = req.user;
  ...
  const kycDocPath = `/assets/KYC documents/${req.file.filename}`;
